...
//Mostrar detalles por factura
router.get("/:id/detalles", detalleFactura);
...
async function detalleFactura(req, res, next) {
    try {
        const items = await controlador.obtenerDetalles(req.params.id);
        respuesta.success(req, res, items, 200);
    } catch (err) {
        next(err);
    }
